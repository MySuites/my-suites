import React from 'react';
import { View, Text, Modal, TouchableOpacity } from 'react-native';
import { IconSymbol, useUITheme } from '@mysuite/ui';

export interface BottomSheetOption {
    value: string;
    label: string;
    icon?: React.ComponentProps<typeof IconSymbol>['name'];
}

interface BottomSheetOptionPickerProps {
    visible: boolean;
    onClose: () => void;
    title: string;
    options: BottomSheetOption[];
    selectedValue?: string;
    onSelect: (value: string) => void;
    defaultIcon?: React.ComponentProps<typeof IconSymbol>['name'];
}

// Bottom-sheet single-select list: backdrop, drag handle, header, then a
// stack of selectable rows (icon + label, checkmark when selected).
export function BottomSheetOptionPicker({
    visible,
    onClose,
    title,
    options,
    selectedValue,
    onSelect,
    defaultIcon = 'gearshape.fill',
}: BottomSheetOptionPickerProps) {
    const theme = useUITheme();

    return (
        <Modal
            visible={visible}
            animationType="slide"
            transparent={true}
            onRequestClose={onClose}
        >
            <TouchableOpacity
                activeOpacity={1}
                onPress={onClose}
                className="flex-1 justify-end"
                style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}
            >
                <TouchableOpacity
                    activeOpacity={1}
                    onPress={(e) => e.stopPropagation()}
                >
                    <View className="bg-light dark:bg-dark-lighter rounded-t-3xl pb-10">
                        {/* Handle bar */}
                        <View className="items-center pt-3 pb-2">
                            <View
                                className="rounded-full bg-black/10 dark:bg-white/10"
                                style={{ width: 36, height: 4 }}
                            />
                        </View>

                        {/* Header */}
                        <View className="flex-row items-center justify-between px-6 pt-2 pb-5">
                            <Text className="text-xl font-bold text-light dark:text-dark">
                                {title}
                            </Text>
                            <TouchableOpacity onPress={onClose} className="p-2">
                                <IconSymbol name="xmark" size={20} color={theme.textMuted || '#888'} />
                            </TouchableOpacity>
                        </View>

                        {/* Options */}
                        <View className="px-4 gap-2">
                            {options.map((opt) => {
                                const isSelected = selectedValue?.toLowerCase() === opt.value.toLowerCase();
                                return (
                                    <TouchableOpacity
                                        key={opt.value}
                                        onPress={() => {
                                            onSelect(opt.value);
                                            onClose();
                                        }}
                                        className={`flex-row items-center justify-between p-4 rounded-2xl ${
                                            isSelected
                                                ? 'bg-primary dark:bg-primary-dark'
                                                : 'bg-black/5 dark:bg-white/5'
                                        }`}
                                    >
                                        <View className="flex-row items-center gap-3">
                                            <IconSymbol
                                                name={opt.icon ?? defaultIcon}
                                                size={16}
                                                color={isSelected ? '#ffffff' : (theme.textMuted || '#888')}
                                            />
                                            <Text
                                                className={`font-semibold text-base ${
                                                    isSelected ? 'text-white' : 'text-light dark:text-dark'
                                                }`}
                                            >
                                                {opt.label}
                                            </Text>
                                        </View>
                                        {isSelected && (
                                            <IconSymbol name="checkmark" size={18} color="#ffffff" />
                                        )}
                                    </TouchableOpacity>
                                );
                            })}
                        </View>
                    </View>
                </TouchableOpacity>
            </TouchableOpacity>
        </Modal>
    );
}
